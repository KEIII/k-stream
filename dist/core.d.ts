...
export declare type SubscribePartialFn<T> = (
  partialObserver: Partial<Observer<T>>
) => Unsubscribable;
export declare type TransformFn<T, O> = (stream: Stream<T>) => Stream<O>;
export declare type PipeFn<T> = <O>(
  transformFn: TransformFn<T, O>
) => Stream<O>;
...
export declare const observerFromPartial: <T>(
  o: Partial<Observer<T>>
) => Observer<T>;
export declare const ksCreateStream: <T>(
 ...
 */
export declare const ksPipe: <A, B, C>(
  t1: TransformFn<A, B>,
  t2: TransformFn<B, C>
) => TransformFn<A, C>;
